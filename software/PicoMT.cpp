...
void * VMUL_Threads_Call(void *pk)
{
  ...
  int         items    = p->len;
  uint32_t    *In1     = p->In1;
  uint32_t    sizeIn1  = p->SizeIn1;
  uint32_t    *In2     = p->In2;
  uint32_t    sizeIn2  = p->SizeIn2;
  uint32_t    *Out     = p->Out;
  uint32_t    sizeOut  = p->SizeOut;
  int err;
  err =   vnew(VM, nPR);                                                        errCheck(err, FUN_VNEW);
  err =   vlpr(VM, nPR->at(0),  A2PB2);                                          errCheck(err, FUN_VLPR);
